...
const WEEKDAY: DayHours = { closed: false, open: '06:00', close: '20:00' };
const SATURDAY: DayHours = { closed: false, open: '08:00', close: '18:00' };
const CLOSED_DAY: DayHours = { closed: true, open: '06:00', close: '20:00' };
const LEGACY_OPEN_DAY: DayHours = { closed: false, open: '07:00', close: '18:30' };
const LEGACY_CLOSED_DAY: DayHours = { closed: true, open: '07:00', close: '18:30' };
...
/** Previous baked-in hours, before the GMB listing was used as the source. */
const LEGACY_DEFAULT_HOURS: HoursInfo = {
  days: {
    monday: { ...LEGACY_OPEN_DAY },
    tuesday: { ...LEGACY_OPEN_DAY },
    wednesday: { ...LEGACY_OPEN_DAY },
    thursday: { ...LEGACY_OPEN_DAY },
    friday: { ...LEGACY_OPEN_DAY },
    saturday: { ...LEGACY_OPEN_DAY },
    sunday: { ...LEGACY_CLOSED_DAY },
  },
...
export function parseStoredHours(raw: unknown): HoursInfo | null {
  ...
  const parsed = { days, note };
  if (isLegacyDefaultHours(parsed)) return DEFAULT_HOURS;
  return parsed;
}
...
function isLegacyDefaultHours(hours: HoursInfo): boolean {
  return (
    hours.note === LEGACY_DEFAULT_HOURS.note &&
    WEEKDAYS.every((weekday) =>
      sameDayHours(hours.days[weekday.id], LEGACY_DEFAULT_HOURS.days[weekday.id])
    )
  );
}
